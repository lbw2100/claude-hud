...
function getStdinCachePath(): string {
  return join(getProjectCacheDir(), 'hud-stdin-cache.json');
}

function getProjectCacheDir(): string {
  const root = getWorktreeRoot() || process.cwd();
  const projectKey = encodeURIComponent(root);
...
export function writeStdinCache(stdin: StatuslineStdin): void {
  try {
    const cacheDir = getProjectCacheDir();
