 ...
 *
 * Statusline command that visualizes HUD state.
...
async function main(watchMode = false): Promise<void> {
  try {
    ...
    try {
      const updateCacheFile = join(homedir(), '.claude', 'hud', 'update-check.json');
    ...
    if (process.env.OMC_DEBUG || process.env.HUD_DEBUG) {
      console.error(
    ...
    if (
      ...
      try {
        const hudStateDir = join(getClaudeConfigDir(), 'hud');
        mkdirSync(hudStateDir, { recursive: true });
        const triggerFile = join(hudStateDir, 'compact-requested.json');
        writeFileSync(
          triggerFile,
          JSON.stringify({
...
// Export for programmatic use (e.g., hud --watch loop)
export { main };
