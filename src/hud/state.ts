...
/**
 * Get the HUD state file path in the project's state directory
 */
function getLocalStateFilePath(directory?: string): string {
  const stateDir = getProjectStateDir(directory);
  return join(stateDir, 'hud-state.json');
}

function getProjectStateDir(directory?: string): string {
  const baseDir = validateWorkingDirectory(directory);
  const projectKey = encodeURIComponent(baseDir);
  return join(getClaudeConfigDir(), 'hud', 'projects', projectKey);
}
...
function getConfigFilePath(): string {
  return join(getClaudeConfigDir(), 'hud', 'hud-config.json');
}
...
export function readHudState(directory?: string): OmcHudState | null {
  // Check new local state first (state/hud-state.json)
  ...
  // Check legacy local state (hud-state.json)
  const baseDir = validateWorkingDirectory(directory);
...
export function readHudConfig(): HudConfig {
  // 1. Try reading from ~/.claude/settings.json (hud key)
  ...
  if (existsSync(settingsFile)) {
    try {
      ...
      if (settings.hud) {
        const config = settings.hud as Partial<HudConfig>;
  ...
  // 2. Try reading from ~/.claude/hud/hud-config.json (legacy)
...
/**
 * Write HUD configuration to ~/.claude/settings.json (hud key)
 */
